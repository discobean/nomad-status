...
if __name__ == '__main__':
    ...
    r = requests.get('%s/v1/agent/self' % (args.agent_url))
    json = r.json()
    node_id = json['stats']['client']['node_id']
    datacenter = json['config']['Datacenter']
    region = json['config']['Region']

    r = requests.get('%s/v1/node/%s' % (args.agent_url, node_id))
    json = r.json()
    ...
    r = requests.get('%s/v1/node/ec29a71d-f68a-27d2-4b34-0207d0648dc8/allocations' % args.agent_url)
    json = r.json()
